...
iris.route.get("/read-group/:gid/:uid", {}, function (req, res) {
  ...
  iris.invokeHook("hook_entity_fetch", 'root', null, fetch).then(function (group) {
    ...
    iris.invokeHook("hook_entity_fetch", thisHook.authPass, null, fetch).then(function (groupResult) {
      ...
      if (groupResult.length > 0) {
        ...
        groupResult.forEach(function (group) {
          ...
          group.field_users.forEach(function(item, key) {
            if (item.field_uid == entity.entityAuthor) {
                group.field_users[key].field_last_checked = group.field_last_updated + 1;
            }
          })
 ...
 */
iris.modules.irisjsMessenger.registerHook("hook_entity_view", 0, function (thisHook, entity) {
